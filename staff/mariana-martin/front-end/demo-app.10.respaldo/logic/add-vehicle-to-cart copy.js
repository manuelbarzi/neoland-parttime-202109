function addVehicleToCart(token, id, callback) {
     ...
     xhr.addEventListener('load', () => {
          ...
          if (status === 401) {
               ...
               const { cart = [] } = payload  //payload en vez de user
               ...
               if (item) //si el item esta ahí, incrementa uno más
                    item.qty++;
               else {  //si no encuentra es que no lo tenía, entonces se crea y empiezo en cantidad 1
                    item = { id, qty: 1 }
               ...
               xhr.addEventListener('load', () => {  //como se usa arrow functions ya no se escribe this...
                    const { status } = xhr
                ...
                xhr.setRequestHeader('Authorization', `Bearer ${token}`)  //template string/interpolar
