...
function retrieveCompletedOrders(userId) {
    ...
    return User.findById(userId)
        .then((user) => {
            ...
            return Order.find({ user: userId, status:'completed' }).populate('items.variant').lean()
        })
        .then(orders => {
...
module.exports = retrieveCompletedOrders