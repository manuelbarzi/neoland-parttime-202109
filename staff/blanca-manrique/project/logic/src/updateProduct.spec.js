...
const { errors: { NotFoundError, AuthError, DuplicityError } } = require('commons')
...
describe('updateProduct', () => {
    ...
    it('should succeed when user, supplier and product already exists', () => {
        return Promise.all([User.deleteMany(), Supplier.deleteMany(), Product.deleteMany()])
            ...
            .then(([user, supplier, product]) => {
                updateProduct(user.id, supplier.id, product.id, '8344444067581263', 'https://www.alibaba.com/product-detail/Wine-Opener-Electric-Opener-Push-type_1600206009330.html?spm=a2700.galleryofferlist.topad_classic.d_image.2ba97c943qedJB', 'Wine opener', 'home and kitchen', 'OEM', 'sustainable one stop', 'stainless steel', 8, 23)
            })
            .then(() => {
                Product.findOne({ supplierProductId: '8344444067581263' })
                    .then(product => {
                        ...
                        expect(product.price).to.equal(8)
                        expect(product.salePrice).to.equal(23)
                    })
            })
    })

    it('should fail when product id from supplier (supplierProductId) is duplicated : supplierProductId is unique', () => {
        return Promise.all([User.deleteMany(), Supplier.deleteMany(), Product.deleteMany()])
            .then(() => {
                const hash = bcrypt.hashSync('121212', 10)
                const user = new User({ username: 'TiGreton', email: 'ti@greton.com', password: hash })
            ...
            .then(([user, supplier1, supplier2, product3, product4]) => { //supplierProductId from product3
                return updateProduct(user.id, supplier2.id, product4.id, '8344444067581263', 'https://www.alibaba.com/product-detail/Wine-Opener-Electric-Opener-Push-type_1600206009330.html?spm=a2700.galleryofferlist.topad_classic.d_image.2ba97c943qedJB', 'Wine opener', 'home and kitchen', 'OEM', 'sustainable one stop', 'stainless steel', 7, 23)
                    .then(() => {
                    ...
                    .catch(error => {
                        ...
                        expect(error.message).to.equal((`already exists a product with the same reference from supplier with id ${product3.supplierProductId}`))
                    })
            })
    })
    ...
    it('should fail when product does not belong to supplier', () => {
        return Promise.all([User.deleteMany(), Supplier.deleteMany(), Product.deleteMany()])
            ...
            .then(([user, supplier1, supplier2, product3, product4]) => {
                return updateProduct(user.id, supplier1.id, product4.id, '975293776332334T', 'https://www.alibaba.com/product-detail/gas-lift-staff-executive-ergonomic-mesh_60758228884.html?spm=a2700.shop_plgr.41413.26.3985515aaki3t1', 'Office Chair', 'office and stationery', 'Jingtian', 'ergonomic mesh office chair', 'polyester mesh', 5, 96)
                    .then(() => {
                        throw new Error('should not reach this point')
                    })
                    .catch(error => {
    ...
    it('should fail when product does not belong to user', () => {
        return Promise.all([User.deleteMany(), Supplier.deleteMany(), Product.deleteMany()])
            ...
            .then(([user1, user2, supplier1, supplier2, product3, product4]) => {
                return updateProduct(user2.id, supplier1.id, product4.id, '975293776332334T', 'https://www.alibaba.com/product-detail/gas-lift-staff-executive-ergonomic-mesh_60758228884.html?spm=a2700.shop_plgr.41413.26.3985515aaki3t1', 'Office Chair', 'office and stationery', 'Jingtian', 'ergonomic mesh office chair', 'polyester mesh', 5, 96)
                    .then(() => {
                        throw new Error('should not reach this point')
                    })
                    .catch(error => {
    ...
    it('should fail when product does not exist', () => {
        ...
        return Promise.all([User.deleteMany(), Supplier.deleteMany(), Product.deleteMany()])
            ...
            .then(([user, supplier]) => updateProduct(user.id, supplier.id, unknownProductId, '975293776332334T', 'https://www.alibaba.com/product-detail/gas-lift-staff-executive-ergonomic-mesh_60758228884.html?spm=a2700.shop_plgr.41413.26.3985515aaki3t1', 'Office Chair', 'office and stationery', 'Jingtian', 'ergonomic mesh office chair', 'polyester mesh', 5, 96))
            .then(() => {
                throw new Error('should not reach this point')
            })
            .catch(error => {
