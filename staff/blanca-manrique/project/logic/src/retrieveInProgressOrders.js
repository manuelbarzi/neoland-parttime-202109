...
function retrieveInProgressOrders(userId) {
    ...
    return User.findById(userId)
        .then((user) => {
            ...
            return Order.find({ user: userId, status:'in progress' }).populate('items.variant').lean()
        })
        .then(orders => {
...
module.exports = retrieveInProgressOrders